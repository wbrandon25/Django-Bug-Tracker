...
def projectPage(request, pk):
    ...
    allTickets = projectObj.bug_ticket_set.filter(Q(title__icontains = q) | Q(description__icontains = q) | Q(author__username__icontains = q) | Q(tag__name__icontains = q) )
    ...
    context = {'number': pk, 'projectObj':projectObj, 'tickets':unresolvedTickets, 'resolvedTickets':resolvedTickets }
...
def userProfile(request, pk):
    ...
    context = {'userObj':userObj,'userComments':userComments, 'tickets':userTickets}
...
def updateProfile(request, pk):
    ...
    if request.method == 'POST':
        form = createExtendedUser(request.POST, request.FILES, instance = extendedUserObj)
        if form.is_valid():
            form.save()
